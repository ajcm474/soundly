...
impl AudioEngine
{
    ...
    /// Get downsampled waveform data for display (legacy method)
    ///
    /// # Parameters
    /// * `samples_per_pixel` - number of audio samples per display pixel
    ///
    /// # Returns
    /// `Vec<(f32, f32)>` - min/max pairs for averaged channels
    ...
    /// Get stereo waveform data for display
    ///
    /// # Parameters
    /// * `samples_per_pixel` - number of audio samples per display pixel
    ///
    /// # Returns
    /// `Vec<(f32, f32, f32, f32)>` - (min_l, max_l, min_r, max_r) tuples
    ///
    /// # Notes
    /// Duplicates mono channel to maintain consistent tuple format
